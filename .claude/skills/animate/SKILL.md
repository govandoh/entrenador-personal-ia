...
> **Adaptación Fitnet.** Skill de Emil Kowalski (MIT, ver `.claude/skills/THIRD-PARTY.md`) instalada en este repo. Antes de aplicarla, lee `.claude/skills/fitnet-diseno/SKILL.md`: sus reglas (tokens de `docs/DESIGN.md`, CSS y WAAPI sin librerías nuevas salvo DEC, presupuesto de la cámara, español en la interfaz) prevalecen sobre este archivo donde choquen. Donde aquí diga "Motion" o recomiende una librería, en Fitnet se usa CSS o WAAPI mientras no exista una DEC que la apruebe.
