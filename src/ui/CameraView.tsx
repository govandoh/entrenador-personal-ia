...
type ActiveExercise = 'squat' | 'curl';
type AnyResult     = SquatResult | BicepCurlResult;
...
export function CameraView() {
      ...
      try {
        ...
        function loop() {
          ...
          if (landmarkSets.length > 0) {
            ...
            const result: AnyResult = ex === 'squat'
              ? squatTrackerRef.current.update(landmarkSets[0])
              : curlTrackerRef.current.update(landmarkSets[0]);
            ...
            if (prevReps >= 0) {
              if (result.reps > prevReps) {
                ...
                if (r.atTop) {
                  speak(r.minAngleReached < GOOD_FORM_ANGLE
  ...
  function handleSwitchExercise() {
    const next: ActiveExercise = activeExRef.current === 'squat' ? 'curl' : 'squat';
