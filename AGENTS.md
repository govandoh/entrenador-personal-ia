...
| `src/domain/{catalog,routineGenerator,sessionHistory,payment}.ts` | Catálogo de 60 ejercicios (DEC-040/056), generador de rutinas por reglas (DEC-056), historial y estadísticas del día, puerto de pagos con `MockPaymentProvider` (regla dura 8). Workstream D. | El historial vive en `localStorage` hasta `api-client`. |
...
| `src/ui/workout/` | Pantalla de entrenamiento (DEC-058): `WorkoutScreen` (cámara, bucle, fases preparación/serie/resumen), `PrepPanel` (nivelador), `SetHud` (isla de aviso, contador), `SetSummaryView`, `coaching.ts` (frases y estrategia de voz), `exercises.ts` (motor por ejercicio, DEC-059). | `WorkoutScreen` concentra cámara, bucle y fases; las frases de técnica usan umbrales de los contadores. |
